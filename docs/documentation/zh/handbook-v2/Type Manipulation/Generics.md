...
在像 C# 和 Java 这样的语言中，创建可重用组件的主要工具之一是_泛型_，即能够创建可以处理多种类型而不仅限于单一类型的组件。这允许用户使用自己的类型来使用这些组件。
...
相反，我们需要一种能够捕获参数类型并在返回类型中使用它的方式。在这里，我们将使用_类型变量_，这是一种特殊类型的变量，用于处理类型而不是值。
...
第二种方式也许是最常用的。这里我们使用_类型参数推断_——即，我们让编译器根据我们传入的实参的类型自动为我们设置 `Type` 的值：
...
如果你还记得之前的例子，有时你可能希望编写一个通用函数，该函数适用于一组类型，你对该类型集合的某些特点有_一些_了解。在我们的 `loggingIdentity` 示例中，我们希望能够访问 `arg` 的 `.length` 属性，但编译器无法证明每种类型都有 `.length` 属性，所以它警告我们不能做出这种假设。
